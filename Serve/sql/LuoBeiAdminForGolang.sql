...
CREATE TABLE `lb_admin` (
  ...
  `interfere` char(32) NOT NULL COMMENT '干扰码',
...
CREATE TABLE `lb_admin_menu` (
  ...
  `api_path` varchar(255) DEFAULT NULL COMMENT '接口地址',
  `characteristic` varchar(255) DEFAULT NULL COMMENT '权限标识',
  `router` varchar(300) DEFAULT NULL COMMENT '路由地址，如果是外链的话这里会变成链接地址',
...
CREATE TABLE `lb_admin_on_line` (
  ...
  `account` char(32) NOT NULL COMMENT 'account',
...
CREATE TABLE `lb_admin_router` (
  ...
  `is_pid` tinyint(1) DEFAULT NULL COMMENT '是否是PID(非用户提交而是通过代码找出来的)',
  `addtime` datetime NOT NULL COMMENT '添加时间',
  PRIMARY KEY (`id`)
...
CREATE TABLE `lb_system` (
  ...
  `tail` varchar(255) NOT NULL COMMENT '网站小尾巴',
  `keyword` varchar(255) NOT NULL COMMENT '网站关键词',
